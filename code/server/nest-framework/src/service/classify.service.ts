...
export class ClassifyService {
   ...
   */
  getNameInfoById(allData: Classify[], lastid: number): any {
    const info = {};
    allData.map(item => {
      if (item.id === lastid) {
        info['name'] = item.name;
   ...
   * @memberof ClassifyService
   */
  getChildrenTree(allData: any[], parentId: number, classifyInfo: any) {
   ...
   * @memberof ClassifyService
   */
  addOne(classify): Promise<Classify> {
