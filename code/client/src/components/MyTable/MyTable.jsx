...
class MyTable extends Component {
  ...
  confirmDelete(id) {
    let newTableList = this.state.tableList.filter((item) => item.id !== id)
    this.setState({
      tableList: newTableList,
    })
    this.props.removeById(id).then(() => {
      if (this.props.messageData !== 0) {
            ...
            render={(text, record) => (
              <Space size="middle">
                ...
                <Popconfirm
                  ...
                  onConfirm={() => this.confirmDelete(text.id)}
